...
try {
	...
	try {
		...
		if(!(_x isKindOf "Man") and (alive _x) and (_x call OT_fnc_hasOwner) and (typeof _x != OT_flag_IND)) then {
			...
			"Failed to save a vehicle... continuing." remoteExecCall ["OT_fnc_notifyAndLog",0,false];
		};
	...
	format["Persistent Save Failed: %1", _exception] remoteExecCall ["OT_fnc_notifyAndLog",0,false];
};
